...
export function TagBadge({ tag, className }: TagBadgeProps) {
  return (
    <Badge
      variant="outline"
      className={`border-0 px-1.5 py-0 text-[10px] leading-5 font-medium capitalize ${TAG_STYLES[tag]} ${className ?? ""}`}
    >
      {TAG_LABELS[tag] ?? tag}
    </Badge>
  )
...
export function CompanyTagBadge({ tag, className }: CompanyTagBadgeProps) {
  return (
    <Badge
      variant="outline"
      className={`border-0 px-1.5 py-0 text-[10px] leading-5 font-medium capitalize ${COMPANY_TAG_STYLES[tag]} ${className ?? ""}`}
    >
